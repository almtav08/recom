...
sys.path.append(".")
from losses.bprloss import BPRLoss
...
def create_batch(batch_indices, X_train, max_seq_length, device):
    ...
    for user in tqdm(range(len(Y_all))):
        ...
        for j in range(num_epochs):
            ...
            for i in range(num_batches):
                ...
                for idx in batch_indices:
                    if idx in positive_indices:
                        new_pos_sample = random.choice(positive_indices)
                        while new_pos_sample == idx:
                            new_pos_sample = random.choice(positive_indices)
                        positive_samples.append(new_pos_sample)

                        new_neg_sample = random.choice(negative_indices)
                        negative_samples.append(new_neg_sample)

                    elif idx in negative_indices:
                        new_pos_sample = random.choice(negative_indices)
                        while new_pos_sample == idx:
                            new_pos_sample = random.choice(negative_indices)
                        positive_samples.append(new_pos_sample)

                        new_neg_sample = random.choice(positive_indices)
                        negative_samples.append(new_neg_sample)

                batches.append(batch_indices)
                batches_positive.append(positive_samples)
                batches_negative.append(negative_samples)
            ...
            for batch_idx, batch_indices in enumerate(batches):
                ...
                anchor_batch, anchor_mask = create_batch(
                    batch_indices, X_train, max_seq_length, device
                )
                positive_batch, positive_mask = create_batch(
                    batches_positive[batch_idx], X_train, max_seq_length, device
                )

                negative_batch, negative_mask = create_batch(
                    batches_negative[batch_idx], X_train, max_seq_length, device
                )

                embeddings = model(anchor_batch, mask=anchor_mask, pooling="last")
                positive_embeddings = model(
                    positive_batch, mask=positive_mask, pooling="last"
                )
                negative_embeddings = model(
                    negative_batch, mask=negative_mask, pooling="last"
                )
