...
from database.orm.async_query_generator import AsyncQueryGenerator
...
def save_relations(course_modules, all_entries):
    ...
    with open('./moodle/data/repeat_graph.json', 'wb') as og:
...
async def load_moodle_info():
    ...
    root = tk.Tk()
    root.title("Indicate resources repeat relations separating the Ids by commas")
